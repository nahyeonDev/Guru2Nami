...
class DetailViewActivity2 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_detailview_tog)

        backBtn = findViewById(R.id.backBtn2)
        goToChat = findViewById(R.id.goToChat2)
        okBtn = findViewById(R.id.okBtn2)
        likeBtn = findViewById(R.id.likeIcon2)

        hopeArea = findViewById(R.id.hopeArea2)
        title = findViewById(R.id.title2)
        content = findViewById(R.id.content2)
        userId = findViewById(R.id.user_id2)
        category = findViewById(R.id.category2)
        imageMain = findViewById(R.id.imgViewMain2)
        ...
        //권한이 허용되지 않음
            if(ActivityCompat.shouldShowRequestPermissionRationale(this,
                ...
                dlg.setPositiveButton("확인"){ dialog, which -> ActivityCompat.requestPermissions(this@DetailViewActivity2,
                        arrayOf(Manifest.permission.READ_EXTERNAL_STORAGE), REQUEST_READ_EXTERNAL_STORAGE)}
                dlg.setNegativeButton("취소", null)
                dlg.show()
            }else {
                //처음 권한 요청
                ActivityCompat.requestPermissions(this@DetailViewActivity2,
                        arrayOf(Manifest.permission.READ_EXTERNAL_STORAGE), REQUEST_READ_EXTERNAL_STORAGE)
            } else {
            //권한이 이미 제대로허용됨
            // getAllPhotos()
        ...
        categ = intent.extras!!.getString("category")
        category.text = categ
        ...
        goToChat.setOnClickListener {
            ...
            Toast.makeText(this@DetailViewActivity2, "채팅에서 만나요 :)", Toast.LENGTH_LONG).show()
        ...
        okBtn.setOnClickListener {
            postBuying()
            val intent = Intent(this, MainActivity::class.java)
            Toast.makeText(this@DetailViewActivity2, "주문 완료!", Toast.LENGTH_LONG).show()
            startActivity(intent)
        }
    ...
    private fun setImage2(url:String) {
